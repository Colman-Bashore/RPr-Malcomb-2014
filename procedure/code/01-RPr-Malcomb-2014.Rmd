...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Colman Bashore, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
`/*{r load DHS survey data, messages=FALSE, warnings=FALSE} dhshh <- readRDS(here(private_r, "datasets", "MWHR61SV.rds")) %>%   zap_labels() head(dhshh)*/`
...
\`\`\`/\*{r load-DHS-geog-data, fig.height = 2} dhsclusters \<- readRDS(here(private_r, "datasets", "MWGE62FL.rds")) %\>% as("sf") \# convert to simple features
...
\`\`\`/{r join-clusters-to-tas} \# spatial join traditional authority ID to clusters and drop geometry \# study may be reanalyzed by buffering GPS points to account for \# GPS location randomization

cluster_ta \<- dhsclusters %\>% st_join(ta_v) %\>% st_drop_geometry() %\>% select(DHSCLUST, ta_id = ID_2, urban_rural = URBAN_RURA)

cluster_ta %\>% saveRDS(here(public_d, "cluster_ta.rds")) /\`\`\`

`/{r load-cluster-ta} cluster_ta <- readRDS(here(public_d, "cluster_ta.rds")) paste(sum(is.na(cluster_ta$ta_id)),   "of", nrow(cluster_ta),    "clusters did not spatially join to a populated traditional authority") %>% noquote()` \`\`\`/{r join-clusterta-to-surveys} \# join traditional authority and urban/rural data from DHS clusters to each DHS survey \# select variables for adaptive capacity analysis dhshh \<- dhshh %\>% left_join(cluster_ta, by = c("HV001" = "DHSCLUST")) %\>% select( HHID, HV001, HV002, ta_id, urban_rural, HV246A, HV246D, HV246E, HV246G, HV248, HV245, HV271, HV251, HV204, HV206, HV226, HV219, HV243A, HV207 )

paste("Starting with", nrow(dhsclusters), "total clusters and", nrow(dhshh), "total household surveys...") %\>% noquote() paste(sum(is.na(cluster_ta$ta_id)),  "clusters did not join to a Traditional Authority, affecting",  sum(is.na(dhshh$ta_id)), "household surveys") %\>% noquote()
...
\`\`\`/{r remove-missing-hh-data} paste(nrow(dhshh), "total household surveys")

hh_capacity_t \<- dhshh %\>% anti_join(incomplete, by = "HHID") %\>% filter(!is.na(ta_id))

paste(nrow(hh_capacity_t), "surveys with complete data") %\>% noquote() paste(nrow(dhshh)-nrow(hh_capacity_t), "surveys with missing attribute data or failed spatial join") %\>% noquote() rm(incomplete)
...
\`\`\`/{r hh-capacity} hh_capacity_t \<- mutate(hh_capacity_t, capacity = 0.04 \* livestock + 0.03 \* sick + 0.06 \* land + 0.04 \* wealth + 0.03 \* orphans + 0.04 \* water + 0.03 \* electricity + 0.02 \* cooking + 0.02 \* femalehh + 0.04 \* cellphone + 0.03 \* radio + 0.02 \* urban )
...
\`\`/{r aggregate-capacity-to-tas} ta_capacity_t \<- hh_capacity_t %\>% group_by(ta_id) %\>% summarize( capacity_avg = mean(capacity), capacity_min = min(capacity), capacity_max = max(capacity), capacity_sd = sd(capacity), n_hh = n(), livestock_avg = mean(livestock), sick_avg = mean(sick), land_avg = mean(land), wealth_avg = mean(wealth), orphans_avg = mean(orphans), water_avg = mean(water), electricity_avg = mean(electricity), cooking_avg = mean(cooking), femalehh_avg = mean(femalehh), cellphone_avg = mean(cellphone), radio_avg = mean(radio), urban_avg = mean(urban), )
...
```{r clip-warp-drought-exposure, warning = F}
names(fl) <- "fl"
...
Switch from raster analysis back into traditional authority boundaries by using a zonal statistic.
The zonal statistic code block will calculate a mean vulnerability score for each traditional authority so that the results can be visualized at the unit of analysis that the authors originally discusses as most important.

```{r mean-vulnerability}
#Calculate the mean vulnerability for each TA using a zonal statistic.
mean_TA_vuln <- aggregate(vulnerability_r, ta_v, FUN = mean) %>% st_as_sf()

```

```{r mean-vulnerability-ta-map, message=F}
ta_vuln_brks <- classIntervals(mean_TA_vuln$vulnerability, 4, style = "jenks")$brks


tmap_mode("plot")
tm_shape(mean_TA_vuln) + 
  tm_polygons(col="vulnerability", breaks=ta_vuln_brks, title="Mean Vulnerability by TA", palette="YlOrRd", lwd=0.1, border.col="lightgrey") +
tm_layout(main.title = "Malawi Climate Change Vulnerability",main.title.size = 1.2, main.title.position = "center", frame = FALSE, legend.width=3, legend.text.size = .7, legend.title.size = 1, asp=0.9, )
...
# Impact of calculating a zonal statistic of vulnerability for each traditional authority

While figure 5 represents the results of the vulnerability model across Malawi using a raster grid, a modification provides a simplification of this visualization.
The most recent modification is a calculation of mean vulnerability for each of the traditional authorities.
This is done through a zonal statistic using the "aggregate" function.
This statistic creates a new polygon layer with traditional authority geometries and a new field called vulnerability than represents the mean vulnerability score for that traditional authority.
The new layer "mean_TA_vuln" is visualized using tmap in a new map that provides another way of conceptualizing vulnerability across the nation of Malawi.
While viewing vulnerability as a raster provides a higher level of detail and follows some of the physical risk dataset types.
However, the modified visualization allows the user to see the vulnerability scores in a more meaningful unit.
Most of the discussion and calculations earlier in the study focused on the traditional authority, so thus it is logical that the final results of the model be shown in the context of traditional authorities.

# Conclusions

The reproduction study attempts to make the steps taken to carry out this study much clearer and more easily replicable.
While the original study presents a novel model there are challenges to be addressed in regards to construct validity and clarity of steps. Therefore, creating a computational notebook that clearly documents all steps taken in this study was a success. Additionally there is more clarity in construct validity and we also showed the vulnerability scores at the unit of the Traditional Authority, which is the most significant administrative unit for implementing change. Thus, this reproduction helps pave the way for more effectively using the Malcomb et al study in new geographic and temporal contexts. 
