...
While figure 5 represents the results of the vulnerability model across Malawi using a raster grid, a modification provides a simplification of this visualization. The most recent modification is a calculation of mean vulnerability for each of the traditional authorities. This is done through a zonal statistic using the "aggregate" function. This statistic creates a new polygon layer with traditional authority geometries and a new field called vulnerability than represents the mean vulnerability score for that traditional authority. The new layer "mean_TA_vuln" is visualized using tmap in a new map that provides another way of conceptualizing vulnerability across the nation of Malawi. While viewing vulnerability as a raster provides a higher level of detail and follows some of the physical risk dataset types. However, the modified visualization allows the user to see the vulnerability scores in a more meaningful unit. Most of the discussion and calculations earlier in the study focused on the traditional authority, so thus it is logical that the final results of the model be shown in the context of traditional authorities. 
